...
function renderReport(result, opts = {}) {
  ...
  const css = `
    ...
    * { box-sizing: border-box; }
    ...
    .disclaimer { background:rgba(202,138,4,.10); border:1px solid var(--moderate);
                  border-left:4px solid var(--moderate); border-radius:8px;
                  padding:14px 18px; margin:0 0 24px; font-size:13px; color:#e8d9a8; }
    .disclaimer strong { color:#fbbf24; }
    .disclaimer p { margin:6px 0 0; color:#cbd5e1; }
  `;

  const disclaimerHtml = `
    <div class="disclaimer">
      <strong>⚠ Automated scan — not a full conformance audit.</strong>
      <p>Automated tools detect roughly <strong>30–50%</strong> of WCAG 2.2 AA issues. A clean result means
      the machine-checkable criteria passed — it does <strong>not</strong> guarantee accessibility or legal
      compliance. Manual testing (keyboard navigation, screen-reader review, logical reading order) is
      still required for full conformance.</p>
    </div>
  `;
